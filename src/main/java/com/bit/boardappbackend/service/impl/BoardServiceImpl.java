...
//            throw new NoSuchElementException("해당 ID의 글이 없습니다.");
//        }
        try {
            boardRepository.deleteById(id); // 얘만 있어도 될듯
        } catch(Exception e) {
    ...
    @Override
    public BoardDto modify(BoardDto boardDto, MultipartFile[] uploadFiles,
                           MultipartFile[] changeFiles, String originFiles, Member member) {
                ...
                if(boardFileDto.getFilestatus().equalsIgnoreCase("U")
                    ...
                    Arrays.stream(changeFiles).forEach(file -> {
                        if(boardFileDto.getNewfilename().equalsIgnoreCase(file.getOriginalFilename())) {
                            ...
                            updateBoardFileDto.setBoard_id(boardFileDto.getId());
